...
int AddGivenLevel(llnode*l1,bstNode*root,int h)
{
    if (root==NULL)
        return -1;
    if (h==1)
        insert_at_end(&l1,root->val);
    if (h>1)
    {
        AddGivenLevel(&l1,root->l,h-1);
        AddGivenLevel(&l1,root->r,h-1);
    }
}


int LLatDepth (bstNode *root)
{
    int h=height(root);
    int i;
    llnode*l1;
